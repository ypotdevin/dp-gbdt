...
class ProbabilityBuckets:
    def __init__(
        self,
        number_of_buckets: int = 100000,
        factor: Optional[float] = None,
        dist1_array=None,
        dist2_array=None,
        caching_directory=None,
        free_infty_budget: float = 10 ** (-20),
        logging_level: int = logging.INFO,
        error_correction: bool = True,
        skip_bucketing: bool = False,
        **kwargs
    ):
        ...
        self.number_of_buckets = self.number_of_buckets = (
            int(4 * (number_of_buckets // 4)) + 1
        )
        self.factor = np.float64(factor)
        self.log_factor = np.log(self.factor, dtype=np.float64)

        # arbitrarily chosen
        self.squaring_threshold_factor = np.float64(1.1)
        self.free_infty_budget = np.float64(free_infty_budget)

        # in case of skip_bucketing = True, caching_setup() has to be
        # called by the derived class as caching depends on a filled
        # self.bucket_distribution
        self.caching_super_directory = caching_directory
        # will be set inside self.caching_setup() if
        # self.caching_super_directory != None
    ...
    def create_bucket_distribution(self, distr1, distr2, error_correction) -> None:
        #
        # For explanation of variables, see comment at the beginning of
        # method "self.compose_with"
        #
        ...
        indices = (
            np.ma.log(np.divide(distr1, distr2, where=~infty_mask)) / self.log_factor
        ...
        if self.error_correction:
            # we want
            #     errors(x,i) = P_B(x) - P_A(x) / f**i
            #                 = P_B(x) - exp( log( P_A(x) ) - i * log_factor )
            errors = distr2 - distr1 / self.factor ** (
                indices - self.number_of_buckets // 2
            )
        else:
        ...
        for i, m_infty, m_null, a, err in zip(
            indices, infty_mask, null_mask, distr1, errors
        ):
            if m_infty:
        ...
        if self.infty_bucket > _infty_bucket_warning_bound:
            self.logger.warning(
                ...
                "Decrease factor or increase number of buckets to avoid this.".format(
    ...
    def squaring(self):
        ...
        if self.error_correction:
            ...
            # By adding self.one_index to this array, we get the
            # corresponding self.bucket_distribution indices.
            # assumptions on next line:
            #     self.one_index % 2 == 1 and
            #     (self.number_of_buckets - 1) % 4 == 0
            n_half_p1_to_n_half = np.arange(
                -(self.one_index // 2) + 1, self.one_index // 2 + 1
            )
            n_half_p1_to_n_half_addr = slice(
                n_half_p1_to_n_half[0] + self.one_index,
                n_half_p1_to_n_half[-1] + self.one_index + 1,
            )
            assert (
                -n_half_p1_to_n_half[0] + 1 == n_half_p1_to_n_half[-1]
            )  # sanity check. If failing: num_of_buckets invalid

            div = lambda i: (1.0 / self.factor ** (2 * i - 1)) - (
                1.0 / self.factor ** (2 * i)
            )
            ...
            def square_error(array):
                ...
                # here we add B(i) * (1/f**(2i-1) - 1/f**(2i) ) for
                # every i in [-n/2+1, n/2]
                temp_error[n_half_p1_to_n_half_addr] += (
                    self.bucket_distribution[
                        (n_half_p1_to_n_half * 2) - 1 + self.one_index
                    ]
                    * div_arr
                )
                return temp_error
        ...
        temp_bucket_distribution[self.one_index // 2 : self.one_index] = collapse(
            lower_half
        )
        temp_bucket_distribution[
            self.one_index + 1 : self.one_index + self.one_index // 2 + 1
        ] = collapse(upper_half)
        temp_bucket_distribution[self.one_index] = self.bucket_distribution[
            self.one_index
        ]
    ...
    def opt_compose_with(
        self, probability_buckets, after_squaring=False, threshold_factor=None
    ):
        assert (
            after_squaring == False and threshold_factor == None
        ), "Function is being called in an unsupported way. We should fix this."
    ...
    def compose_with(self, pb, allow_modify_instance=False):
        ...
        ), "ERROR, trying to compose distributions with different factors"

        # we want to compute the product of the two probability buckets
        # instances A and B: I_A.I_B  ( . denotes a "inner" product)
        # let A (or B) denote the bucketlist and A_infty (or B_infty)
        # denote the infinty bucket: I_A = (A, A_infty), I_B = (B, B_infty)
        ...
        #
        # as in the computation of A.B under- and overflows can occur,
        # we get in the end six terms:
        #
        # I_A.I_B = (A.B)_under + A.B + (A.B)_over + A_infty + B_infty - A_infty * B_infty
        #
        # The first term goes to the first bucket, the second is the
        # bucket distribution, and the last 4 terms we pack in the infty
        # bucket
        #
        # UPDATE: there are now two "infty buckets":
        #     self.distinguishing_events - keeps track of the real distinguishing events
        #     self.infty_bucket - containing numerical overswap of A.B,
        #                         and the mix terms of disting_events
        #                         and infty_bucket.
        #
        # Technically, there are two terms added to self.infty_bucket:
        #     -A_infty*B_dist - A_dist*B_infty
        # resulting from (A, A_infty, A_dist).(B, B_infty, B_dist) .

        # add "A_dist + B_dist - A_dist * B_dist"
        self.distinguishing_events = (
            self.distinguishing_events
            + pb.distinguishing_events
        ...
        temp_infty_bucket += (
            self.infty_bucket + pb.infty_bucket - (self.infty_bucket * pb.infty_bucket)
        )
        temp_infty_bucket += (
            -self.infty_bucket * pb.distinguishing_events
            - self.distinguishing_events * pb.infty_bucket
        )
        ...
        while True:
            ...
            temp_over = self.convolve_full(
                self.bucket_distribution[self.one_index + 1 :],
                pb.bucket_distribution[self.one_index + 1 :],
            )

            # add all infty parts together:
            # "(A.B)_over + A_infty + B_infty - A_infty * B_infty"
            delta_infty_bucket = (
                np.sum(temp_over[self.one_index - 1 :]) + temp_infty_bucket
            )

            max_growth_allowed = self.squaring_threshold_factor * (
                self.infty_bucket + pb.infty_bucket
            )
            if (
                delta_infty_bucket >= self.free_infty_budget
                and delta_infty_bucket > max_growth_allowed
            ):
                self.squaring()
                if self.bucket_distribution is not pb.bucket_distribution:
                    if (
                        not allow_modify_instance
                    ):  # make a copy so we do not change the original instance
                        pb = pb.copy()
        ...
        temp_bucket_distribution[1:] = self.convolve_same(
            self.bucket_distribution, pb.bucket_distribution
        )[1:]

        # compute the first bucket (A.B)_under
        self.logger.info("   Compute (A.B)_under")
        temp_under = self.convolve_full(
            self.bucket_distribution[0 : self.one_index + 1],
        ...
        if self.error_correction:
            ...
            factors = np.exp(
                self.log_factor * np.arange(-self.one_index, self.one_index + 1)
            )
            ...
            temp_buck_distr_B_convolved = convert_to_B(
                temp_bucket_distribution, factors
            )
            ...
            # We compute the latter one because it involves only one
            # convolution and the subtraction term we already know

            self.logger.info("   Compute real_error A.B")
            self.real_error = (
                self.convolve_same(
                    temp_buck_distr_B_self + self.real_error,
                    temp_buck_distr_B_pb + pb.real_error,
                )
                - temp_buck_distr_B_convolved
            )
            self.real_error[0] = 0

            self.logger.info("   Compute virtual_error A.B")
            temp_buck_distr_B_virt_err_self = (
                temp_buck_distr_B_self + self.virtual_error
            )
            temp_buck_distr_B_virt_err_pb = temp_buck_distr_B_pb + pb.virtual_error
            temp_virtual_error = (
                self.convolve_same(
                    temp_buck_distr_B_virt_err_self, temp_buck_distr_B_virt_err_pb
                )
                - temp_buck_distr_B_convolved
            )

            self.logger.info("   Compute virtual_error A.B_under")
            temp_virtual_error_under = self.convolve_full(
                temp_buck_distr_B_virt_err_self[0 : self.one_index + 1],
                temp_buck_distr_B_virt_err_pb[0 : pb.one_index + 1],
            )
            temp_virtual_error_under = np.sum(
                temp_virtual_error_under[0 : self.one_index + 1]
            )
    ...
    def load_state(cls, state_directory):
        ...
        instance.bucket_distribution = np.fromfile(
            os.path.join(state_directory, "bucket_distribution"), dtype=np.float64
        )
        if instance.error_correction:
            instance.real_error = np.fromfile(
                os.path.join(state_directory, "real_error"), dtype=np.float64
            )
            instance.virtual_error = np.fromfile(
                os.path.join(state_directory, "virtual_error"), dtype=np.float64
            )
    ...
    def save_state(self, state_directory):
        ...
        # excluding np arrays from dumping in "non_ndarray" without
        ...
        self.bucket_distribution.tofile(
            open(os.path.join(state_directory, "bucket_distribution"), "w")  # type: ignore
        )
        if self.error_correction:
            self.real_error.tofile(
                open(os.path.join(state_directory, "real_error"), "w")  # type: ignore
            )
            self.virtual_error.tofile(
                open(os.path.join(state_directory, "virtual_error"), "w")  # type: ignore
            )

    def compose(self, nr_of_compositions: int):
        if not self.caching_directory:
            self.logger.critical("This method requires caching. Abort")
            raise RuntimeError("This method requires caching. Abort")

        state_filepath_base = os.path.join(self.caching_directory, "compositions-")
        get_state_filepath = lambda needed_exp: state_filepath_base + str(
            int(2**needed_exp)
        )
        ...
        # which compositions do we need
        target_exp = int(np.log2(nr_of_compositions))
        needed_compositions = [
            x if (nr_of_compositions & (2**x) != 0) else -1
            for x in range(target_exp + 1)
        ]
        needed_compositions = list(filter(lambda a: a != -1, needed_compositions))

        self.logger.info(
            "Needed compositions: " + ", ".join(map(str, needed_compositions))
        )
        ...
        for needed_exp in range(target_exp + 1):
            ...
            if not os.path.isdir(state_filepath):
                self.logger.info(
                    "[*] State 2**" + str(needed_exp) + " does not exist. Creating it."
                )
                if not needed_exp == 0:
                    if avoided_self_composition:
                        # only load from disk when it differs from
                        # current state
        ...
        self.logger.info(
            "[*] All intermediate states up to 2**" + str(target_exp) + " exist now"
        )
        ...
        for i in needed_compositions[1:]:
            ...
            # while the factor of previous state and current state is
            # not same
            while current_state.factor != previous_state.factor:
                self.logger.info(
                    "factors are unequal ( {} != {} ), squaring".format(
                        current_state.factor, previous_state.factor
                    )
                )
    ...
    def delta_PDP(self, eps):
        ...
        if self.error_correction:
            self.logger.warning(
                "Error correction for PDP delta not supported. Omit error correction."
            )

        k = int(np.floor(eps / self.log_factor))

        if k > self.number_of_buckets // 2:
            # eps is above of bucket_distribution range
            return self.infty_bucket + self.distinguishing_events
        if k < -self.number_of_buckets // 2:
            # eps is below of bucket_distribution range
            k = -self.number_of_buckets // 2

        return (
            np.sum(self.bucket_distribution[self.one_index + k + 1 :])
            + self.infty_bucket
            + self.distinguishing_events
        )
    ...
    def delta_ADP(self, eps):
        """Returns an upper bound for tight delta for approximate
        differential privacy. Error correction is supported."""
    ...
    def delta_ADP_upper_bound(self, eps):
        ...
        ret = np.sum(
            self._g_func(np.arange(1, self.one_index - k + 1))
            * self.bucket_distribution[self.one_index + k + 1 :]
        )
        if self.error_correction:
            ret -= np.exp(eps) * np.sum(
                self.real_error[
                    min(self.one_index + k + self.u, self.number_of_buckets) :  # type: ignore
                ]
            )
    ...
    def eps_ADP_upper_bound(self, delta) -> float:
        """
        ...
        if not self.error_correction:
            self.logger.warning(
                "Error-correction disabled, optimizer might not converge properly."
            )
        ...
        try:
            root = optimize.bisect(
                lambda eps: self.delta_ADP_upper_bound(eps) - delta, 0, max_eps
            )
        except ValueError as e:
            if self.delta_ADP_upper_bound(eps=max_eps) > delta:
                raise ValueError(
                    "Required target-delta is smaller than self.delta_ADP_upper_bound(max_eps) can serve. "
                    "For an instant remedy, increase number_of_buckets, or increase factor."
                )
            elif self.delta_ADP_upper_bound(eps=0.0) < delta:
                self.logger.warning(
                    "Returning over-approximation eps=0. "
    ...
    def renyi_divergence_upper_bound(self, alpha):
        ...
        assert (
            self.distinguishing_events == 0 and self.infty_bucket == 0
        ...
        # the alpha renyi moments are the alpha-1 moments of the
        # exponentiated bucket_distribution. Informal, i.e.
        #     R(alpha) = 1/lam * ln E_{y~buckets} exp(y*lam)
        #
        # for additional details, see Lemma 8 in
        # Sommer et al. "Privacy loss classes: The central limit theorem
        # in differential privacy." PoPETS 2019.2

        # to provide a upper bound, we assume that all content of a
        # specific bucket manifests at the position with the highest
        # leakage (most right)
        summands = np.exp(
            (np.arange(self.number_of_buckets) - self.one_index) * self.log_factor * lam
        )
    ...
    def KL_divergence(self):
        return np.sum(
            self.bucket_distribution
            * ((np.arange(self.number_of_buckets) - self.one_index) * self.log_factor)
        )
...
class ProbabilityBuckets_fromDelta(ProbabilityBuckets):
    ...
    def __init__(self, delta_func, DP_type, **kwargs):
        """
        delta_func: callable that returns a tight ADP or PDP delta for
        (positive and NEGATIVE) epsilons DP_type = 'adp' | 'pdp' for
        aproximate or probabilistic differential privacy respectivly.
        """

        # Tell the parent __init__() method that we create our own
        # bucket_distribution.
        kwargs["skip_bucketing"] = True

        # Our custom create_bucket_distribution() method does not set up
        ...
        # Caching setup needs to be called after the buckets have been
        # filled as the caching utilized a hash over the bucket
        # distribution
        self.caching_setup()
    ...
    def create_bucket_distribution(self, delta_func, DP_type):
        ...
        # the eps vector we use for delta generation. We should have one
        # value within
        eps_vec = (
            np.arange(self.number_of_buckets + 1) - self.number_of_buckets // 2 - 1
        ) * self.log_factor

        # The deltas we search to mimic with our future distribution.
        delta_vec = [delta_func(eps) for eps in eps_vec]

        # Generating G, the coefficient matrix for which we try to solve
        # min_w  G.dot(w) - delta_func(eps_vec)
        try:
            if DP_type == "adp":
                # assuming delta_func describes approximate differential
                # privacy

                # y: our base points (right edge of each bucket)
                y = (
                    np.arange(self.number_of_buckets) - self.number_of_buckets // 2
                ) * self.log_factor
                ...
                G = np.append(
                    G, np.ones((self.number_of_buckets + 1, 1)), axis=1
                )  # for the infty bucket

            elif DP_type == "pdp":
                # assuming delta_func describes probabilistic
                ...
                raise NotImplementedError(
                    "DP_type '{}' not implemented.".format(DP_type)
                )

        except MemoryError as e:
            raise MemoryError(
                "Insufficient memory. Use smaller number_of_buckets."
            ) from e

        # we try to solve min_w  G.dot(w) - delta_func(eps_vec) with
        #     w_i > 0 forall i
        ...
        # distinguishing_events is zero as we cannot distinguish between
        # infty bucket and distinguishing events
        self.distinguishing_events = np.float64(0.0)

        # and some internal stuff
        # this is a shortcut to the 0-bucket where L_A/B ~ 1
        self.one_index = int(self.number_of_buckets // 2)
