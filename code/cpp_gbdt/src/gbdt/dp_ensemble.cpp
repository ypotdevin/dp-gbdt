...
DPEnsemble::DPEnsemble(ModelParams *parameters) : params(parameters)
{
    ...
    {
        std::call_once(flag1, []()
                       { std::cout << "!!! DP disabled !!! (slower than dp!)" << std::endl; });
        params->use_dp = false;
...
void DPEnsemble::train(DataSet *dataset)
{
    ...
    for (int tree_index = 0; tree_index < params->nb_trees; tree_index++)
    {
        ...
        { // build a dp-tree
             ...
             */
            // Is it really OK to predict on the whole dataset? What are the
            // privacy budget implications?
            ...
            {
                // TODO: Discuss if it is allowed not to remove the tree_indices
                // from the dataset (does it have hidden privacy costs?).
                trees.pop_back();
            }
        }
        else
